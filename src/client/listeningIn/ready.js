...
module.exports = class Ready {
  ...
  async run() {
    ...
    cron.schedule('*/20 * * * *', async () => {
      ...
      await allGuilds.forEach(async (asd) => {
      ...
      if (timeout - (Date.now() - server.bolsa.tempo) > 0) {
        ...
        this.client.channels.cache.get('915649663208661062').send(embed);
      }

      await allItens.forEach(async (e) => {
        await allGuilds.forEach(async (i) => {
