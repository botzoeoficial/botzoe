	...
	async run({
		...
		if (timeout - (Date.now() - server.cidade.eleicao.cooldown) > 0) {
			...
			message.channel.send(author, embed).then(async (msg) => {
				...
				const sim = await msg.createReactionCollector((r, u) => r.emoji.name === '✅' && u.id === author.id, {
					time: 180000,
					max: 1
				});

				const não = await msg.createReactionCollector((r, u) => r.emoji.name === '❌' && u.id === author.id, {
					time: 180000,
					max: 1
				});
				...
				sim.on('collect', async () => {
					...
					await msg.edit(author, embed).then(async (msg3) => {
