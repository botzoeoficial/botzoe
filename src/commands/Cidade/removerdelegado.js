...
module.exports = class Removerdelegado extends Command {
	...
	async run({
		...
		if (member.user.bot) {
			return message.reply({
				content: 'Um bot não nunca irá ser Delegado desse servidor.'
			});
		}

		if (server.cidade.delegado !== member.id) {
			return message.reply({
				content: 'Esse usuário não é o Delegado desse servidor.'
			});
		}
		...
		return message.reply({
			content: `O usuário ${member} saiu do cargo de Delegado desse servidor com sucesso.`
		});
	}
